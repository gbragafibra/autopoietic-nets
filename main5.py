...
##---- Variable radius for each unit
μ = 2 #mean
σ = 2 #std
radius = np.random.normal(loc = μ, scale = σ, size = (N, N))#radius for consideration
...
def update(frame, *args):
    ...
    ax1.set_title(f"Threshold (ε): {ε}; Fixed (k: {k if not fix else None}): {fix}; Φ $\geq$ ε: {geq_cond}; μ: {μ}; σ: {σ}")
    return mat1, mat2
