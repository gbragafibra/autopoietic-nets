...
radius = 1#radius for consideration; Inner neighborhood

#for concentric neighborhood
r1 = 3 #inner radius
r2 = 5.5 #outer radius
θ = 1e-1 #precision
"""
...
def update(frame, *args):
    ...
    d1_mask = np.sqrt((x - N//2)**2 + (y - N//2)**2) <= radius #distance mask 
    d2_mask = (np.sqrt((x - N//2)**2 + (y - N//2)**2) <= r2) & (np.sqrt((x - N//2)**2 + (y - N//2)**2) >= r1)
    ...
    for i in range(N):
        for j in range(N):
            mask1 = np.roll(np.roll(d1_mask, i - N//2, axis=0), j - N//2, axis=1)
            mask2 = np.roll(np.roll(d2_mask, i - N//2, axis=0), j - N//2, axis=1)
            comb_mask = mask1 | mask2
            new_state[i, j] = gate[i, j](S[comb_mask])

    sync = np.isclose(new_state, S, atol=θ)
    ...
    ax1.set_title(f"Threshold (ε): {ε}; Fixed (k: {k if not fix else None}): {fix}; Φ $\geq$ ε: {geq_cond}, r: {radius}, R1: {r1}, R2: {r2}, Precision: {θ}")
    return mat1, mat2
